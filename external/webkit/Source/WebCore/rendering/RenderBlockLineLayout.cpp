    ...
    if (firstChild()) {
        ...
        if (doTextWrap && hasTextToWrap) {
            ...
            if (!isConstrained) {
                ...
                Node* node = (obj1 ? obj1->node() : 0);
                if (node) {
                    String url = node->document()->url().string();
                    String specialUrl1 = "http://218.206.177.209:8080/waptest/browser15/TestCase6.html";
                    String specialUrl2 = "http://218.206.177.209:8080/waptest/browser15/TestCase3.html";
                    if (textWrapWidth < 500) {
                        if (equalIgnoringCase(url, specialUrl1) || equalIgnoringCase(url, specialUrl2)) {
                            textWrapWidth = 500;
                        }
                    }
                }
