...
static void wpas_send_action_done(void *ctx)
{
	...
	wpabuf_free(wpa_s->pending_action_tx);
	wpa_s->pending_action_tx = NULL;
	if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) && 0) {
...
#else
    os_memcpy(p2p.p2p_dev_addr, wpa_s->own_addr, ETH_ALEN);
...
int wpas_p2p_set_value(struct wpa_supplicant *wpa_s, struct wpa_driver_test_params *params)
{
	return wpa_drv_set_value(wpa_s, params);
}
