...
unsigned int bl_div = CLK_DIV128; //CLK_DIV1 at 20121130
...
int backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	printk("[qbq] kernel: backlight set gpio pwm mode\n");
	
		//printk("[LED]PWM con register is %x \n", INREG32(PWM_BASE + 0x0150));
	return 0;
}

//baoqiang add for camry
static struct PWM_config resume_pwm = {0};
static u32 user_level=130;
int resume_set_backlight(void)
{
	u32 lcd_bl_level;
	lcd_bl_level = brightness_mapping(user_level);
	//lcd_bl_level =  brightness_mapping(g_leds_data[MT65XX_LED_TYPE_TOTAL-1]->level);
	//lcd_bl_level = (g_leds_data[MT65XX_LED_TYPE_TOTAL-1]->level + 1)/4;
	//printk("----qbq---%s--lcd_level=%d--user_level=%d--\n",__func__,lcd_bl_level,user_level);
	msleep(15);
	
	backlight_set_pwm(3,lcd_bl_level,0,&resume_pwm);
	//printk("----qbq---%s---\n",__func__);
...
extern bool LCM_RESUME_FLAG ; //baoqiang add 
		...
		case MT65XX_LED_MODE_PWM:
			if(strcmp(cust->name,"lcd-backlight") == 0)
			{
				...
				if(level!=0){
					if(level !=20)
						user_level = level;
				}//to save user backlight level		
				
				//printk("-----%s--flag=%d--user_level=%d---\n",__func__,LCM_RESUME_FLAG,user_level);
				if(LCM_RESUME_FLAG)
				{					
					LCM_RESUME_FLAG=FALSE;
					return 0;					
				}
				*/
				//baoqiang add end
...
static void mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	struct mt65xx_led_data *led_data =
       ...
       if(!strcmp(led_data->cust.name,"button-backlight"))
       	{
           if(level < 1)
       	    {
	...
	if ((led_data->level != level)||!strcmp(led_data->cust.name,"button-backlight")) {
...
static ssize_t store_notify_led(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	struct cust_mt65xx_led *cust_led_list = get_cust_led_list();
        ...
        led_tmp_setting.blink_off_time=delayon;
        led_tmp_setting.blink_on_time=delayoff;
