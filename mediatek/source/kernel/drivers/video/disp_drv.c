...
DISP_STATUS DISP_PanelEnable(BOOL enable)
{
	...
	if (enable && !s_enabled) {
		...
		if(lcm_params->type==LCM_TYPE_DSI && lcm_params->dsi.mode != CMD_MODE)
		{
			...
			//msleep(100);
		#ifdef BUILD_UBOOT
			DISP_CHECK_RET(DISP_UpdateScreen(0,0,540,960)); //baoqiang add. when device rusume, transfer one black data to avoid short time white screen.
			msleep(200);
		#else
			msleep(2);
		#endif
...
BOOL DISP_EsdCheck(void)
{
    ...
    if(lcm_drv->esd_check == NULL && disp_drv->esd_check == NULL) //legen 0922
    {
        return FALSE;
    }
...
BOOL DISP_EsdRecoverCapbility(void)
{
    ...
    if((lcm_drv->esd_check && lcm_drv->esd_recover) || (lcm_params->dsi.lcm_ext_te_monitor) || (lcm_params->dsi.lcm_int_te_monitor)) // legen 0922
    {
        return TRUE;
    }
    else
