...
extern bool is_hdmi_active();
extern void hdmi_update();
...
static int mtkfb_set_s3d_ftm(struct fb_info *info, unsigned int mode)
{
	...
	struct fb_overlay_layer layerInfo;
	//uint8_t framebuffer[1080*960*3];  ----two layer buffer,each one is 540*960*3
...
#ifdef CONFIG_HAS_EARLYSUSPEND
bool LCM_RESUME_FLAG = FALSE; //baoqiang add 
//extern int resume_set_backlight( u32 level);
#if 0
static void mtkfb_set_backlight(void)
{
	int count = 0;
...
static void mtkfb_late_resume(struct early_suspend *h)
{
	...
	if(BL_set_level_resume){
		//mtkfb_set_backlight_level(BL_level); //no need to set backlight here, the type to set backlight here isn't ours.
		BL_set_level_resume = FALSE;
		}

	//baoqiang add at 20121030
	//LCM_RESUME_FLAG = TRUE;
	
	DISP_CHECK_RET(DISP_UpdateScreen(0, 0, fb_xres_update, fb_yres_update));	
	//baoqiang add end
	msleep(10);
    //resume_set_backlight(); //turn on backlight
	//mtkfb_set_backlight();
