...
int MT6620_RDS_Reset_Block_Counter(void);
...
static int rds_group2_rt_addr_bitmap_get_pos(uint16_t bitmap)
{
    ...
    for (j = i; j >= 0; j--) {
        if (!(bitmap & (1 << j))) {
            FM_LOG_NTC(D_RDS, "uncomplete msg 0x%04x, delete it\n", bitmap);
            return -1;
        }
    }
#endif
...
static int MT6620_RDS_RetrieveGroup2(uint16_t *source, uint8_t subtype, RDSData_Struct *target)
{
    ...
    while(1){
        switch(rds_rt_state_get()){
            ...
            if ((txt_end == TRUE) //find 0x0D, and the lenth has been recorded when do rds_group2_rt_cmp()
                        || (rds_group2_rt_addr_bitmap_get(bitmap_twice) == 0xFFFF) //get max  64 chars
            ...
            case RDS_RT_GETLEN:
                ...
                if (rt_len > 0 && ((txt_end == TRUE) || (rds_group2_rt_addr_bitmap_get(bitmap_twice) == 0xFFFF))) {
