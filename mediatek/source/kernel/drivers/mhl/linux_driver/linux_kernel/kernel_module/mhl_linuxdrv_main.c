...
{
	pTaskStruct = kthread_run(EventThread,
											 &gDriverContext,
											 MHL_DRIVER_NAME);
}
...
static unsigned int rcpkey[]={
	MHL_RCP_CMD_SELECT, 			KEY_SELECT,
	MHL_RCP_CMD_UP, 				KEY_UP,
	MHL_RCP_CMD_DOWN, 			KEY_DOWN,
	MHL_RCP_CMD_LEFT, 			KEY_LEFT,
	MHL_RCP_CMD_RIGHT, 			KEY_RIGHT,
	MHL_RCP_CMD_ROOT_MENU, 		KEY_MENU,
	MHL_RCP_CMD_EXIT, 			KEY_BACK,
	MHL_RCP_CMD_NUM_0,			KEY_0,
	MHL_RCP_CMD_NUM_1, 			KEY_1,
	MHL_RCP_CMD_NUM_2, 			KEY_2,
	MHL_RCP_CMD_NUM_3, 			KEY_3,
	MHL_RCP_CMD_NUM_4, 			KEY_4,
	MHL_RCP_CMD_NUM_5, 			KEY_5,
	MHL_RCP_CMD_NUM_6, 			KEY_6,
	MHL_RCP_CMD_NUM_7, 			KEY_7,
	MHL_RCP_CMD_NUM_8, 			KEY_8,
	MHL_RCP_CMD_NUM_9, 			KEY_9,
	MHL_RCP_CMD_DOT, 				KEY_DOT,
	MHL_RCP_CMD_ENTER, 			KEY_ENTER,
	MHL_RCP_CMD_CLEAR, 			KEY_CLEAR,
	MHL_RCP_CMD_SOUND_SELECT, 	KEY_SOUND,
	MHL_RCP_CMD_PLAY, 			KEY_PLAY,
	MHL_RCP_CMD_PAUSE, 			KEY_PAUSE,
	MHL_RCP_CMD_STOP, 			KEY_STOP,
	MHL_RCP_CMD_FAST_FWD, 		KEY_FASTFORWARD,
	MHL_RCP_CMD_REWIND, 			KEY_REWIND,
	MHL_RCP_CMD_EJECT, 			KEY_EJECTCD,
	MHL_RCP_CMD_FWD, 			KEY_NEXTSONG,
	MHL_RCP_CMD_BKWD, 			KEY_PREVIOUSSONG,
	MHL_RCP_CMD_PLAY_FUNC, 		KEY_PLAY,
	MHL_RCP_CMD_PAUSE_PLAY_FUNC, 	KEY_PLAYPAUSE,
	MHL_RCP_CMD_STOP_FUNC, 		KEY_STOP,
	MHL_RCP_CMD_F1, 				KEY_F1,
	MHL_RCP_CMD_F2, 				KEY_F2,
	MHL_RCP_CMD_F3, 				KEY_F3,
	MHL_RCP_CMD_F4, 				KEY_F4,
	MHL_RCP_CMD_F5, 				KEY_F5,
};

static unsigned int rcpkey_longpressed[]={
	MHL_RCP_CMD_FAST_FWD,
	MHL_RCP_CMD_REWIND,
};

static void rcp_key_longpressed_delay(uint8_t rcp_keycode, unsigned long ms)
{
	int i;

	for(i=0; i<ARRAY_SIZE(rcpkey_longpressed); i++)  {
		if(rcp_keycode == rcpkey_longpressed[i]){
			msleep(ms);
			break;
		}
	}
...
extern struct input_dev* get_mhl_input_dev(void);

static void input_report_rcp_key(uint8_t rcp_keycode, int up_down)
{
	unsigned int i;
	unsigned int rpt_keycode = KEY_RESERVED;
	struct input_dev * mhl_rcp_input_dev = get_mhl_input_dev();	
	
	rcp_keycode &= 0x7F;
	for(i=0; i<ARRAY_SIZE(rcpkey); i+=2)
	{
		if(rcp_keycode == rcpkey[i]){
			rpt_keycode = rcpkey[i+1];
			break;
		}
	}

	TX_DEBUG_PRINT(("rcp_keycode=%d,rpt_keycode=%d\n",rcp_keycode,rpt_keycode));

	if(mhl_rcp_input_dev == NULL){
		TX_DEBUG_PRINT(("error: mhl_rcp_input_dev == NULL!!!\n"));
		return;
	}
	
	if(rpt_keycode != KEY_RESERVED){
		input_report_key(mhl_rcp_input_dev, rpt_keycode, up_down);
		input_sync(mhl_rcp_input_dev);		
	}
}

static void input_report_mhl_rcp_key(uint8_t rcp_keycode)
{
	//added  for improving mhl RCP start
	input_report_rcp_key(rcp_keycode & 0x7F, 1);	
	rcp_key_longpressed_delay(rcp_keycode & 0x7F, 300);
