...
#if !defined(__KERNEL__)
static SiiResultCodes_t CraReadBlockI2c ( prefuint_t busIndex, uint8_t deviceId, uint8_t regAddr, uint8_t *pBuffer, uint16_t count )
{
...
static SiiResultCodes_t CraWriteBlockI2c ( prefuint_t busIndex, uint8_t deviceId, uint8_t regAddr, const uint8_t *pBuffer, uint16_t count )
{
    SiiResultCodes_t status = SII_ERR_FAIL;
    do {
        if ( I2cSendStart( busIndex, deviceId, &regAddr, 1, false ) != PLATFORM_SUCCESS )
...
void SiiRegWrite ( SiiReg_t virtualAddr, uint8_t value )
{
    ...
    switch ( pPage->busType )
    {
        ...
        case DEV_I2C_2:
        case DEV_I2C_3:
            status = CraWriteBlockI2c( pPage->busType, (uint8_t)pPage->address, regOffset, &value, 1 );
            break;
        case DEV_I2C_OFFSET:
        case DEV_I2C_1_OFFSET:
        case DEV_I2C_2_OFFSET:
        case DEV_I2C_3_OFFSET:
            status = CraWriteBlockI2c( pPage->busType - DEV_I2C_OFFSET , (uint8_t)pPage->address, regOffset + (uint8_t)(pPage->address >> 8), &value, 1 );
            break;
        default:
            break;
    }
