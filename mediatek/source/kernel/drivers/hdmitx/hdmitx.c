...
static unsigned int dpi_clk_div, dpi_clk_duty, hsync_pulse_width, hsync_back_porch, hsync_front_porch, vsync_pulse_width, vsync_back_porch, vsync_front_porch, mipi_pll_clk_ref, mipi_pll_clk_div1, mipi_pll_clk_div2, intermediat_buffer_num;
...
void hdmi_source_buffer_switch(void)
{
    ...
    RETIF_NOLOG(IS_HDMI_NOT_ON(), 0);
    RETIF_NOLOG(p->output_mode == HDMI_OUTPUT_MODE_DPI_BYPASS, 0);
    RETIF_NOLOG(IS_HDMI_IN_VIDEO_MODE(), 0);
...
void hdmi_update(void)
{
    ...
    wake_up_interruptible(&hdmi_update_wq); //wait up hdmi_update_kthread() to do update
#else
    hdmi_update_impl();
#endif
}
void hdmi_update_impl(void)
{
    ...
    {
        memset(hdmi_va, 0, p->hdmi_width*p->hdmi_height*3*hdmi_params->intermediat_buffer_num);
    ...
    switch(pmdp.orientation)
    {
        ...
        case 2:
        {
            ...
            //pmdp.dstAddr = hdmi_va + hdmi_buffer_write_id*p->hdmi_width*p->hdmi_height*3+(p->hdmi_width - pmdp.dstW)/2*3;
            pmdp.dstAddr = hdmi_va + hdmi_buffer_write_id * p->hdmi_width * p->hdmi_height * 3   +  
            	(p->hdmi_height - pmdp.dstH) / 2 * p->hdmi_width * 3+ (p->hdmi_width - pmdp.dstW) / 2 * 3;
...
/*static*/ HDMI_STATUS hdmi_drv_deinit(void)
{
	...
	m4u_invalid_tlb_range_stub(M4U_CLNTMOD_LCDC, temp_mva,temp_mva+p->lcm_width*p->lcm_height*3*2);
	ret = m4u_dealloc_mva_stub(M4U_CLNTMOD_LCDC, temp_va, p->lcm_width*p->lcm_height*3*2, temp_mva);
	if(ret)
	{
		printk("[hdmi]allocate mva for dpi fail\n");
		return -1;
    ...
    m4u_invalid_tlb_range_stub(M4U_CLNTMOD_DPI, hdmi_mva, hdmi_mva+p->hdmi_width*p->hdmi_height*3*2);


	ret = m4u_dealloc_mva_stub(M4U_CLNTMOD_DPI, hdmi_va, p->hdmi_width*p->hdmi_height*3*2, hdmi_mva);
	if(ret)
	{
		printk("[hdmi]allocate mva for dpi fail\n");
		return;
...
void hdmi_state_callback(HDMI_STATE state)
{
    ...
    RETIF((p->is_force_disable == true), 0);
    RETIF(IS_HDMI_FAKE_PLUG_IN(), 0);
...
void test_hdmi_enable(bool enable)
{
	if(enable)
	{
		hdmi_ioctl((struct file*)NULL, MTK_HDMI_AUDIO_VIDEO_ENABLE, 1);
	}
	else
	{
		hdmi_ioctl((struct file*)NULL, MTK_HDMI_AUDIO_VIDEO_ENABLE, 0);
	}
}
