...
# add by zhoujinggao
ifeq ($(ENABLE_32CPU_BUILD),true)
MAKEJOBS     :=  -j32
endif

ifeq ($(ENABLE_16CPU_BUILD),true)
MAKEJOBS     :=  -j16
endif
...
ifneq ($(PROJECT),generic)
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) mediatek/source/dct/DrvGen mediatek/custom/$(PROJECT)/kernel/dct/$(PRIVATE_CUSTOM_KERNEL_DCT)/codegen.dws $(DEAL_STDOUT_DRVGEN) && \
	 $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
	 $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
endif
