...
if ($emmc_support eq "yes")
{
  ...
  push(@lnmatrix,"mediatek/build/tools/ptgen/MT6577/mobile_info.img");
  push(@lnmatrix,"mediatek/build/tools/ptgen/MT6577/fat.img");
	opendir (DIR,"mediatek/source/misc");
	@dir = readdir(DIR);
	foreach $temp (@dir){
		if($temp=~/(EBR\d)/){
			print "~~~~~~~~~copy  $temp $1\n";
			push(@lnmatrix,"mediatek/source/misc/$1");
		}
	}
	closedir DIR;
}
