...
if($PLATFORM eq "MT6583"){
	$SHEET_NAME = $PLATFORM;	
}

if($PLATFORM eq "MT8320"){
	if($EMMC_SUPPORT eq "yes"){
		$SHEET_NAME = $PLATFORM ." emmc" ;
	}else{
		$SHEET_NAME = $PLATFORM ." nand";
	}	
}
...
#convert dec start_address to hex start_address
	...
	for($row=1;$row < @PARTITION_FIELD;$row++){
		if($PARTITION_FIELD[$row] eq "BMTPOOL" || $PARTITION_FIELD[$row] eq "OTP"){
			$START_FIELD_Byte_HEX[$row] = sprintf("FFFF%04x",$START_FIELD_Byte[$row]/(128*1024));#$START_FIELD_Byte[$row];
		}else{
...
#
	...
	my @ebr_start_block;
	my $ebr_count = 0;
	my $br_folder = "mediatek/source/misc/";
my @BR = (
	["mediatek/source/misc/MBR", [	[0x00,0x0,0x0],
					[0x00,0x0,0x0],
					[0x00,0x0,0x0],
					[0x00,0x0,0x0]]],
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] =~ /EBR(\d)/){
			...
			$BR[$1][0] = "mediatek/source/misc/"."$PARTITION_FIELD[$iter]";
...
__TEMPLATE
	...
	for ($iter=0; $iter< $total_rows; $iter++){
		...
		if($PARTITION_FIELD[$iter] eq "PRELOADER" ||$PARTITION_FIELD[$iter] eq "DSP_BL" ||$PARTITION_FIELD[$iter] eq "UBOOT"){
