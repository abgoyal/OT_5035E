...
public:
        ...
        virtual bool streamForcedToSpeaker(int streamType);
        virtual bool streamForcedAudible(int streamType);
        virtual bool streamForcedToSpeakerandMute(int streamType);
        virtual bool streamMuteInRingTone(int streamType);
        virtual bool streamMuteInForceSpeaker(int streamType);
        virtual void SetStreamMuteForceSpeaker(bool enable ,int mHardwareOutput, int delay =0);
        virtual bool IsOutputDeviceWiredOn(void);
        virtual bool ForcedAudioNeedMute(audio_io_handle_t output);
        virtual uint32_t  GetMultiplier(uint32_t volume,uint32_t dBAttenTotal);
        virtual uint32_t GetMapVolumeIndex(uint32_t volume);

#ifdef WITH_A2DP
        virtual bool routedToA2dpOutput(int streamtype);
        virtual void setA2dpEnabled(AudioSystem::device_connection_state state);
...
protected:
        ...
        uint32_t mForcedSpeakerCount;
        uint32_t mForcedAudibleCount;
        uint32_t mForcedSpeakerMuteCount;
        //fm speaker status_t
...
public:
        ...
        AUDIO_GAIN_TABLE_STRUCT Audio_gaintable_Custom_Volume;
        #endif
	...
	#ifdef MTK_AUDIO_GAIN_TABLE_SUPPORT
	float ComputegainTableVolume(int stream , float &vol);
	float MapGaintableVoltoCustomVol(AUDIO_GAIN_CONTROL_STRUCT array[], int volmin, int volmax, int &vol, int VolumeMaxLevel);
	#endif
	void LoadCustomVolume(void);
	status_t SetPolicyManagerParameters(int par1,int par2 ,int par3,int par4);

	// for Audio gain tables
	void InitVolumeIndex_UCM(AudioSystem::stream_type stream,int indexMin,int indexMax);
	void SetStreamIndex_UCM(audio_io_handle_t output,AudioSystem::stream_type stream,int index);
	void SetStreamStart_UCM(audio_io_handle_t output,AudioSystem::stream_type stream);
	void SetStreamStop_UCM(audio_io_handle_t output,AudioSystem::stream_type stream ,int delay=0);
private:
