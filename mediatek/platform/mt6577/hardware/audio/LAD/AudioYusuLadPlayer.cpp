...
#define LOG_TAG "LADPlayBuffer"
    ...
    #define LOG_LADPlay LOGD
#else
    #define LOG_LADPlay LOGV
#endif
...
//   p_OutFile1 = fopen("/data/Record_before_BLI.pcm","wb");
   ...
   if( p_OutFile1 == NULL)
   {
      LOGD("Fail to Open Record_before_BLI File %s ",p_OutFile1);
   }
...
LADPlayer::LADPlayer(AudioYusuHardware *hw)
{
   ...
   if ( ret != 0 )
       LOGD("Failed to initialize mLadPlayerWriteMutex components for LADPlayer!");
...
//   pOutFile = fopen("/data/Record_BGS.pcm","wb");
   if( pOutFile == NULL)
   {
      LOGD("Fail to Open RecBGS File %s ",pOutFile);
   }
#endif
...
LADPlayBuffer* LADPlayer::LADPlayer_CreateBuffer(uint32 sampleRate, uint32 chNum, int32 format, uint32 targetSR)
{
   ...
   LOGD("LADPlayer_CreateBuffer sampleRate=%d ,ch=%d, format=%d, targetSR:%d",sampleRate,chNum,format, targetSR);
...
//            LOGD("LADPlayer_DoMix buf underflow dataCountInBuf:%d ",dataCountInBuf);
         }
         //unlock
   ...
   if ( activeBufCount  == 0 ) {
      LOGD("-LADPlayer_DoMix activeBufCount:%d, dataCountInBuf:%d ",activeBufCount,dataCountInBuf);
      dataCountInBuf = 0;
   }
...
//   int32 inc = WriteShareBufHeader(pBuf1, (int16)type, (int16)requiredDataCount, (int16)A2M_BUF_HEADER);
   ...
   if(dataCountInBuf == 0){
      LOGD("--LADPlayer_DoMix dataCountInBuf(0) ");
...
bool LADPlayer::LADPlayer_CheckBufferEmpty()
{
    ...
    LOGV("LADPlayer_CheckBufferEmpty return ret = %d",ret);
    return ret;
}
