...
#include "AudioStreamInHandler.h"
    ...
    #define LOG_STREAMHANDLER LOGD
#else
    #define LOG_STREAMHANDLER LOGV
#endif
...
{
    LOGD("FindDeviceWithMux device = %d",device);
    if(device == android_audio_legacy::AudioSystem::DEVICE_OUT_EARPIECE){
        ...
        LOGW("FindDeviceWithMux warning ");
        return AUDIO_PATH;
    }
}
...
void AudioStreamHandler::AudioStartRunning(unsigned int OutStreamIndex,unsigned int InterruptCounter)
{
   ...
   LOGD("AudioStartRunning mCurrentDevice = 0x%x", mCurrentDevice);
   ...
   {
       LOGD("comunication_status  mAfe_handle->Afe_DL_Start();");
       int device = mAudioHardware->GetCurrentDevice ();
...
void AudioStreamHandler::AudioStopRunning()
{
   ...
   /*
   LOGD("AudioStopRunning mCurrentDevice = 0x%x mPreViousDevice = %x prestate = %d",
   ...
   if(mAudioHardware->getPreviousMode() == android_audio_legacy::AudioSystem::MODE_IN_COMMUNICATION &&
          (mPreViousDevice == android_audio_legacy::AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_HEADSET || mPreViousDevice == android_audio_legacy::AudioSystem::DEVICE_OUT_BLUETOOTH_SCO )&&(!voice_status)){
...
{
   mSrcBlockRunningNum++;
   LOGD("AddSrcRunningNumber mSrcBlockRunningNum:%d",mSrcBlockRunningNum);
}

void AudioStreamHandler::MinusSrcRunningNumber(void)
{
   mSrcBlockRunningNum--;
   LOGD("MinusSrcRunningNumber mSrcBlockRunningNum:%d",mSrcBlockRunningNum);
}

int AudioStreamHandler::GetSrcRunningNumber(void)
{
   LOGD("GetSrcRunningNumber, mSrcBlockRunningNum:%d",mSrcBlockRunningNum);
	...
	uint32 devices,int *format, uint32_t *channelCount, uint32_t *sampleRate, status_t *status)
{
   ...
   LOGD("openOutputStream, format=%d ,channels=%d, rate=%d",*format,*channelCount,*sampleRate);
...
{
    LOGD("StoreInputStreamAttribute (%d, %u, %u)", *format, *channelCount, *sampleRate);
...
{
   int Stream_index = -1;
   LOGD("+openInputStream devices = %x format = %d channelCount = %d samplerate = %d",
   ...
   {
      LOGD("openInputStream devices == android_audio_legacy::AudioSystem::DEVICE_IN_AUX_DIGITAL");
      // no input available
      Stream_index = this->FindFreeInputStream();
      if(Stream_index < 0 || Stream_index >=MAX_INPUT_STREAM_NUM)
      {
         LOGD("openInputStream :: no free stream available");
         return NULL;
      }
   ...
   LOGD("set return lStatus = %d",lStatus);
   ...
   LOGD("-openInputStream, mInput create");
...
void  AudioStreamHandler::closeInputStream(android_audio_legacy::AudioStreamIn* in)
{
   ...
   LOGD("closeInputStream:index=%x",Stream_index);
   if(Stream_index <0){
      LOGD("closeInputStream:no match instream");
   }
   else{
      if(mInput[Stream_index] != NULL ){
         LOGD("closeInputStream +Hdl lock");
...
{
   LOGD("::SetCloseRec mPreviousMode=%d, mMode=%d",mPreviousMode,mMode);
   return RecordClose();
}

bool AudioStreamHandler::SetOpenRec(int mPreviousMode,int mMode)
{
   LOGD("::SetOpenRec mPreviousMode = %d , mMode = %d",mPreviousMode,mMode);
   return RecordOpen();
}
...
void AudioStreamHandler::SetOutputSamplerate(uint32 samplerate)
{
   ...
   for(int i=0; i< MAX_OUTPUT_STREAM_NUM ; i++)
   {
      if(mOutput[i]!= NULL)
      {
         ...
         if(ret <0)
            LOGE("::SetOutputSamplerate ERROR!!!");
      }
   }
}
...
void AudioStreamHandler::SetOutputStreamToBT(void)
{
    ...
    if(GetSrcBlockRunning ()){
        ...
        int interval = (ASM_BUFFER_SIZE *1000) / (BTSCO_SAMPLERATE*2);
