...
static void FM_I2S_Setting()
{
    ...
    if(fd_FM < 0){
        LOGE("Open 'dev/fm' Fail !fd_FM(%d)", fd_FM);
    }
    else
    {
        ret_FM = ioctl(fd_FM, FM_IOCTL_I2S_SETTING, &Setting_FM);
        if(ret_FM){
            LOGE("set ioctl FM_IOCTL_I2S_SETTING Fail !ret_FM(%d)", ret_FM);
        }
    }
    close(fd_FM);
}
...
bool AudioAfe::Afe_Set_Stream_Attribute(uint32 format,uint32 channelCount,uint32 sampleRate)
{
   ...
   if(format == android_audio_legacy::AudioSystem::PCM_16_BIT){
      ...
      LOGE("Afe_Set_Stream_Attribute error format=%d \n", format);
      result = false;
   }
   ...
   if(channelCount ==android_audio_legacy::AudioSystem::CHANNEL_OUT_MONO){
      ...
      mCh = channelCount;
  	  ...
  	  LOGE("Afe_Set_Stream_Attribute error ch=%d \n", channelCount);
      result = false;
   }
   ...
   switch(sampleRate)
   {
      ...
      case 48000:
         mSamplingRate = sampleRate;
        	break;
      default:
         LOGE("AudioAfe::Afe_Set_Stream_Attribute error sampleRate=%d \n", sampleRate);
         result = false;
    }
    return result;
}
...
bool AudioAfe::Afe_Set_Stream_Attribute(uint32 format,uint32 channelCount,uint32 sampleRate, AFE_MODE Open_Mode)
{
    ...
    switch(Open_Mode){
        ...
        default:
            LOGW("Afe_Set_Stream_Attribute default");
    }
     return true;
}
...
{
    LOGV("SetAWBConenction Connect = %d",Connect);
    if(Connect){
...
{
    LOGV("SetAWBEnable Enable = %d bitmask = %x",Enable,bitmask);
    uint32 val = 0;
    uint32 reg =0;
...
{
    LOGD("Afe_Set_FmTx_AnalogOut, Enable=%d \n",Enable);
    if(Enable >0){
    //set interconenct
...
void AudioAfe::Afe_Set_FmTx_DigitalOut(int32 Enable, int32 DAC_Reopen)
{
    ...
    LOGD("Afe_Set_FmTx_DigitalOut, Enable=%d \n",Enable);
    if(Enable >0){
...
void AudioAfe::Afe_DL_Start(AFE_MODE OpenMode)
{
   ...
   if(OpenMode == AFE_MODE_DAC)
   {
      ...
      LOGD("Afe_DL_Start AFE_MODE_DAC SR_index = %d SR_index_mode = %d",SR_index,SR_index_mode);
   ...
   else if(OpenMode == AFE_MODE_I2S1_OUT_HDMI)  // DL1 --> I2S1 output
   {
      ...
      SR_index = Afe_Get_DL_SR_Index(mSamplingRate);
      SR_index_mode = SamplingRateConvert[SR_index];
      LOGD("Afe_DL_Start, HDMI, SR_index_mode(%d)",SR_index_mode);
...
//      SetAfeReg(AFE_SDM_GAIN_STAGE,0x0000001e,0xffffffff);
      ...
      // to do
      LOGD("Afe_DL_Start, AUD_SET_HDMI_SR (%d) \n",mSamplingRate);
      ...
      GetAfeReg(AFE_DL_SRC2_1,&val);
      LOGD("Afe_DL_Start, HDMI +AFE_DL_SRC2_1 (0x%x) \n",val);
      SetAfeReg(AFE_DL_SRC2_1,(SR_index << 28),0xf0000000);  // bit31~bit28:  DL2_INPUT_MODE_SEL
   ...
   else if(OpenMode == AFE_MODE_DAI)
   {
       ...
       LOGD("Afe_DL_Start, AFE_DAC_CON1, AFE_MODE_DAI value = %x",value);
       SetAfeReg(AFE_DAC_CON1,value,0x000b000f);      // bit19: DL1_OPMODE  1:Master mode , 0:Slave mode
       ...
       LOGD("Afe_DL_Start, -AFE_DAIBT_CON0 (0x%x) \n",val);
       val &= ~(1<<8);// high word
   ...
   else if(OpenMode == AFE_MODE_FTM_I2S)
   {
      ...
      if(mt6573_chip_ver == 0x8a00){
         LOGD("Afe_DL_Start, AFE_MODE_FTM_I2S, MT6573 E1 Chip (slave mode) ");
         SetAfeReg(AFE_I2S_IN_CON,0x1d,0x1f);   // bit0: 1:enable I2S input
      }
      else if(mt6573_chip_ver == 0xca10){
         LOGD("Afe_DL_Start, AFE_MODE_FTM_I2S, MT6573 E2 Chip (master mode) ");
         SetAfeReg(AFE_I2S_IN_CON,0x19,0x1f);   // bit0: 1:enable I2S input
      }
      else // default use master mode
      {
         LOGD("Afe_DL_Start, AFE_MODE_FTM_I2S, No Chip ver (master mode) ");
         SetAfeReg(AFE_I2S_IN_CON,0x19,0x1f);   // bit0: 1:enable I2S input
      }
...
{
   LOGD("Afe_UL_Start OpenMode=%d",OpenMode);
   ...
   if(OpenMode == AFE_MODE_ADC)
   {
      ...
      LOGD("Afe_UL_Start AFE_MODE_ADC SR_index_mode = %d",SR_index_mode);
   ...
   else if(OpenMode == AFE_MODE_I2S_IN)
   {
      ...
      {
         // Turn on I2S ON
         LOGD("Afe_UL_Start, AFE_DAC_CON0, I2S ON \n");
         SetAfeReg(AFE_DAC_CON0,0x20,0x20);  //bit5: I2S ON
      }
      usleep(100);
      if(afe_on == 0)
   ...
   else if(OpenMode == AFE_MODE_AWB)
   {
       ...
       SR_index_mode = SamplingRateConvert[SR_index];
       LOGD("Afe_UL_Start AFE_MODE_AWB SR_index = %d SR_index_mode = %d",SR_index,SR_index_mode);
       // Set connection
...
{
   LOGD("!!! Afe_DL_Stop OpenMode=%d",OpenMode);
   uint32 value = 0x0;
   uint32 mask  = 0x0;
   int sph_on = false;
   int bgs_on = false;
   ...
   if(OpenMode == AFE_MODE_DAC)
   {
      ...
      LOGD("Afe_DL_Stop, AFE_IRQ_CON, Disable interrupt \n");
      ...
      LOGD("Afe_DL_Stop, sph_on:%d, bgs_on:%d, rec_on:%d, vt_on:%d \n",sph_on,bgs_on,rec_on,vt_on);
//      if( (sph_on==false) && (bgs_on==false) && (rec_on==false) && (vt_on==false) )
   ...
   {
       // Disable interrupt (timer)
       LOGD("Afe_DL_Stop  AFE_MODE_I2S0_OUT, AFE_IRQ_CON, Disable interrupt \n");
       SetAfeReg(AFE_IRQ_CON,0x0,0x1);
   ...
   else if(OpenMode == AFE_MODE_I2S1_OUT_HDMI)  // for HDMI use
   {
      //------------------------------------------------------------------
      // Disable interrupt (timer)
      LOGD("Afe_DL_Stop  HDMI, Disable interrupt \n");
      SetAfeReg(AFE_IRQ_CON,0x0,0x1);
      ...
      LOGD("Afe_DL_Stop, HDMI, sph_on:%d, bgs_on:%d, rec_on:%d, vt_on:%d \n",sph_on,bgs_on,rec_on,vt_on);
   ...
   else if(OpenMode == AFE_MODE_DAI)
   {
       ...
       GetAfeReg(AFE_DAIBT_CON,&val);
       LOGD("Afe_DL_Stop, GetAfeReg AFE_DAIBT_CON = %x",val);
       val &=~ (1<<1);  // BT_ff
       val &= ~(1); // disable DAI_BT
       val &= ~(1<<3); // disable DAI_BT
       SetAfeReg(AFE_DAIBT_CON,val,0xffff); // enable for DAI_BT function , BT_ON=1 , data ready =1
       LOGD("Afe_DL_Stop, AFE_MODE_DAI, Disable interrupt val = %d",val);
       GetAfeReg(AFE_DAIBT_CON,&regvalue);
       LOGD("Afe_DL_Stop, GetAfeReg AFE_DAIBT_CON = %x",regvalue);
   ...
   LOGD("Afe_DL_Stop, AFE_DAC_CON0 (0x%x) \n",val_dac);
...
#endif		   	   	
   {
      LOGD("Afe_DL_Stop, AFE_DAC_CON0, Disable AFE_ON \n");
      SetAfeReg(AFE_DAC_CON0,0x0,0x1); // AFE_ON
   }
}
...
void AudioAfe::Afe_UL_Stop(AFE_MODE OpenMode)
{
   ...
   if(OpenMode == AFE_MODE_ADC)
   {
      ...
      // Disable interrupt (timer)
      LOGD("Afe_UL_Stop, AFE_IRQ_CON, Disable interrupt \n");
   ...
   uint32 val_dac = 0x0;
   GetAfeReg(AFE_DAC_CON0,&val_dac);
   LOGD("Afe_UL_Stop, AFE_DAC_CON0 (0x%x) \n",val_dac);
   if( (val_dac & 0x7E) == 0 )
   {
      //LOGD("Afe_UL_Stop, AFE_DAC_CON0, Disable AFE_ON \n");
      SetAfeReg(AFE_DAC_CON0,0x0,0x1); // AFE_ON
   }
}
...
bool AudioAfe::SetI2SControl(bool bEnable,int type,uint32 SampleRate)
{
    if(mI2SInstance == NULL){
        LOGE("SetI2SControl mI2SInstance == NULL");
        return false;
    }
    if(bEnable == true){
...
void AudioAfe::Afe_Set_Stereo(void)
{
   ...
   {
       LOGD("!!! Afe_Set_Stereo is forced to Afe_Set_Mono().\n");
       SetAfeReg(AFE_CONN1,(1<<27),(1<<27));  // Connect -- DL1_R to DAC_L, bit27: I06-O03
       SetAfeReg(AFE_CONN2,(1<<17),(1<<17));  // Connect -- DL1_L to DAC_R, bit17: I05-O04
...
void AudioAfe::Afe_Set_Mono(void)
{
   ...
   LOGD("!!! MTK_AUDIO_SW_S2M  Afe_Set_Mono \n");
   Afe_Set_Stereo();
   mAudioStereoToMono = true;
...
{
   LOGD("HQA_AFE_Set_UL_ADC_PGA_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_AFE_Set_UL_ADC_Minima_PGA_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_AFE_Set_I2S_PATH, I2SRec_Enable:0x%x \n",I2SRec_Enable);
