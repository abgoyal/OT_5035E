...
#if 1//Added by jrd.lipeng for martell mini test headset loopback issue.
static char hdst_loopback_enabled = 0;
#endif
...
void AudioFtm::WavGen_AudioRead(char * pBuffer, unsigned int bytes)
{
   ...
   LOGV("WavGen_AudioRead IdxAudioPattern=%d, SizeAudioPattern=%d, copysize=%d, bytes=%d,", IdxAudioPattern, SizeAudioPattern, copysize, bytes);
   ...
   while(bytes){
      ...
      LOGV("in loop IdxAudioPattern=%d, SizeAudioPattern=%d, copysize=%d, bytes=%d,", IdxAudioPattern, SizeAudioPattern, copysize, bytes);
   }
...
void AudioFtm::FmTx_thread_digital_out(void)
{
   ...
   LOGD("FmTx_thread_digital_out: SizeByte=%d, g_i2VDL_DATA addr=0x%x", SizeByte, g_i2VDL_DATA);
   ...
   while (1)
   {
      if(!mAudioSinWave_thread)
      {
         ...
         LOGD("break FmTx_thread_digital_out thread");
         break;
      }
...
void  AudioFtm::FmTx_thread_analog_out(void)
{
   ...
   LOGD("FmTx_thread_analog_out: SizeByte=%d, g_i2VDL_DATA addr=0x%x", SizeByte, g_i2VDL_DATA);
   ::ioctl(mFd, AUD_SET_CLOCK, 1);
   ::ioctl(mFd, START_DL1_STREAM, 0);
   ...
   while (1)
   {
      if(!mAudioSinWave_thread)
      {
         ...
         ::ioctl(mFd, AUD_SET_CLOCK, 0);
         LOGD("break FmTx_thread_analog_out thread");
         break;
      }
      //Read pattern and write to asm buf
      WavGen_AudioRead(mAudioBuffer, SizeByte);
...
void AudioFtm::HDMI_thread_I2SOutput(void)
{
   ...
   LOGD("HDMI_thread_I2SOutput: SizeByte=%d, g_i2VDL_DATA addr=0x%x", SizeByte, g_i2VDL_DATA);
   ...
   //set gpio 53/54/55
//   LOGD("HDMI_thread_I2SOutput, AUD_SET_HDMI_GPIO");
//   ::ioctl(mFd, AUD_SET_HDMI_GPIO, 1);// enable FM use I2S
   ...
   while (1)
   {
      if(!mAudioSinWave_thread)
      {
         ...
         mAfe->SetAfeReg(AFE_DL_SRC1_1, 0, 0xf);
         //reset gpio
         LOGD("HDMI_thread_I2SOutput, AUDDRV_RESET_BT_FM_GPIO");
//         ::ioctl(mFd, AUD_SET_HDMI_GPIO, 0);// Reset GPIO pin mux
         ::ioctl(mFd, STANDBY_DL1_STREAM, 0);

         ::ioctl(mFd, AUD_SET_HDMI_CLOCK, 0);
         LOGD("exit HDMI_thread_I2SOutput thread");
         break;
      }
      //Read pattern and write to asm buf
      WavGen_AudioRead(mAudioBuffer, SizeByte);
...
{
   uint32 Reg_Freq, Reg;
   LOGD("WavGen_SW_SineWave, Enable=%d,  Freq = %d\n", Enable, Freq);
   ...
   if(Enable)
   {
      if(!mAudioSinWave_thread)
      {
         ...
         {
            LOGE("Invalid SineWave Frequency %d", Freq);
            return false;
         }
         else
         {
   ...
   {//WavGen_SWPattern
      ...
      {
         //leave thread
         LOGD("WavGen_SWPattern, destroy m_WaveThread");
         mAudioSinWave_thread = false;
...
{
   LOGD("Afe_Enable_SineWave, bEnable:%d \n",bEnable);
   uint32 value = 0x0;
   uint32 mask  = 0x0;
   if( bEnable )
   {
...
{
    LOGD("FTM_AFE_UL_Loopback, bEnable:%d \n",bEnable);
...
#if 1//Added by jrd.lipeng for martell mini test headset loopback issue.
        if(hdst_loopback_enabled == true)
            digital_mic &= ~DIGITAL_MIC1;
#endif
...
#if 1//Added by jrd.lipeng for martell mini test headset loopback issue.
        if(hdst_loopback_enabled == false)
#endif
...
{
   LOGD("FTM_AFE_Loopback, bEnable:%d \n",bEnable);
   if(bEnable == true)
   {
...
{
   LOGD("Meta_Open_Analog pinmux=%d \n",pinmux);
...
{
    LOGD("FTM_Ana_Loopback, bEnable:%d \n",bEnable);
    if(bEnable)
    {
...
{
    LOGD("Audio_Set_Speaker_On Channel = %d\n",Channel);
    if (mFd>0)
    {
    ...
    {
        LOGD("Set_Speaker_On with mFd not init\n");
    }
}

void AudioFtm::Audio_Set_Speaker_Off(int Channel)
{
    LOGD("Audio_Set_Speaker_Off Channel = %d\n",Channel);
    if (mFd>0)
    {
    ...
    {
        LOGD("Set_Speaker_OFF with mFd not init\n");
    }
}

void AudioFtm::Audio_Set_HeadPhone_On(int Channel)
{
    LOGD("Audio_Set_HeadPhone_On Channel = %d\n",Channel);
    if (mFd>0)
    {
    ...
    {
        LOGD("Audio_Set_HeadPhone_On with mFd not init\n");
    }
}

void AudioFtm::Audio_Set_HeadPhone_Off(int Channel)
{
    LOGD("Audio_Set_HeadPhone_Off Channel = %d\n",Channel);
    if (mFd>0)
    {
    ...
    {
        LOGD("Audio_Set_HeadPhone_Off with mFd not init\n");
    }
}

void AudioFtm::Audio_Set_Earpiece_On()
{
    LOGD("Audio_Set_Earpiece_On Channel\n");
    if (mFd>0)
    {
    ...
    {
        LOGD("Audio_Set_Earpiece_On with mFd not init\n");
    }
}

void AudioFtm::Audio_Set_Earpiece_Off()
{
    LOGD("Audio_Set_Earpiece_Off Channel\n");
    if (mFd>0)
    {
    ...
    {
        LOGD("Audio_Set_Earpiece_Off with mFd not init\n");
    }
}
...
void AudioFtm::Aud_Loopback(char b_phone_mic_sel,
            ...
            char b_headset_out)
{
    LOGD("!!! Aud_Loopback phone_mic=%d, headset_mic=%d, Spk=%d, Receiver=%d, headset=%d"
                ,b_phone_mic_sel,b_headset_mic_in,b_spk_out,b_receiver_out,b_headset_out);
#if 1//Added by jrd.lipeng for martell mini test headset loopback issue.
        hdst_loopback_enabled = !!b_headset_mic_in;
#endif
    usleep(200*1000);
    if ((b_phone_mic_sel==MIC1_ON) || (b_phone_mic_sel==MIC2_ON))
    {
        ...
        {
            mAnalog->SetAnaReg(AUDIO_CON1,0x0400,0x3f00);//set HPL Gain
            mAnalog->SetAnaReg(AUDIO_CON1,0x0004,0x003f);//set HS Gain
...
{
    LOGD("PhoneMic_Receiver_Loopback echoflag=%d",echoflag);
...
{
    LOGD("PhoneMic_EarphoneLR_Loopback echoflag=%d",echoflag);
...
{
    LOGD("PhoneMic_SpkLR_Loopback echoflag=%d",echoflag);
...
{
    LOGD("HeadsetMic_EarphoneLR_Loopback, bEnable=%d",bEnable);
...
{
    LOGD("HeadsetMic_SpkLR_Loopback echoflag=%d",echoflag);
...
{
    LOGD("+SetLoopbackOn(%d, %d)", eLoopbackType, bEarphoneInserted);

    // check no loobpack function on
    pthread_mutex_lock_timeout_np(&mLoopbackMutex, 1000); // 1s
    if (eCurrentLoopbackType != NO_LOOPBACK) 
    {
        LOGW("Please Turn off Loopback Type %d First!!", eCurrentLoopbackType);
    ...
    bool bPreviousHAL_DualMic_Setting = false;
    if (bMdLoopback == true) 
    {
        // check mLad
        if (mLad == NULL) 
        {
            LOGW("mLad == NULL!! return");
            pthread_mutex_unlock(&mLoopbackMutex);
            return false;
        }

        // set output device by bEarphoneInserted
        eOutputDevice = (bEarphoneInserted) ? LADOUT_SPEAKER2 : LADOUT_SPEAKER1;

        // let both earphone L/R channel have sound
        bConnectAllADDA = (bEarphoneInserted) ? true : false;

#if defined(MTK_DUAL_MIC_SUPPORT)
        // temporally disable mHAL_DualMic_Setting to avoid LAD_SetInputSource set main mic => dual mic
        if (mAudioHardware != NULL)
        {
            bPreviousHAL_DualMic_Setting = mAudioHardware->mHAL_DualMic_Setting;
            mAudioHardware->mHAL_DualMic_Setting = false;
        }
#endif
        // mute
    ...
    switch(eLoopbackType)
    {
        ...
        case MD_MAIN_MIC_ACOUSTIC_LOOPBACK:
            ...
            break;
        case MD_HEADSET_MIC_AFE_LOOPBACK:
            mLad->LAD_SetInputSource(LADIN_Microphone2);
            mLad->LAD_SetOutputDevice(eOutputDevice);
            mLad->LAD_SetAfeLoopback(true, bSwapADDA, bConnectAllADDA);
            break;         
        case MD_HEADSET_MIC_ACOUSTIC_LOOPBACK:
        ...
        case MD_DUAL_MIC_AFE_LOOPBACK:
            ...
            mLad->LAD_SetInputSource(LADIN_DualAnalogMic);
            mLad->LAD_SetOutputDevice(eOutputDevice);
            mLad->LAD_SetAfeLoopback(true, bSwapADDA, bConnectAllADDA);
            break;  
        case MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITHOUT_DMNR:
        ...
        case MD_BT_LOOPBACK:
            ...
            break;
        default:
            LOGW("-SetLoopbackOn(%d) : No such Loopback type", eLoopbackType);
    ...
    {
#if defined(MTK_DUAL_MIC_SUPPORT)        
        // recover mHAL_DualMic_Setting
        if (mAudioHardware != NULL) mAudioHardware->mHAL_DualMic_Setting = bPreviousHAL_DualMic_Setting; 
#endif
...
{
    LOGD("+SetLoopbackOff(), eCurrentLoopbackType = %d", eCurrentLoopbackType);

    pthread_mutex_lock_timeout_np(&mLoopbackMutex, 1000); // 1000 ms

    if (eCurrentLoopbackType == NO_LOOPBACK)
    {
        LOGD("-SetLoopbackOff() : No looback to be closed");
        pthread_mutex_unlock(&mLoopbackMutex);        
        return true;
    }
