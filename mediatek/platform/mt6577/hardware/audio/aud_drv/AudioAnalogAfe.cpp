...
AudioAnalog::AudioAnalog(AudioYusuHardware *hw)
{
   ...
   if ( ret != 0 ) LOGE("Failed to initialize pthread depopMutex!");
...
void AudioAnalog::AnalogAFE_Open(AnalogAFE_Mux pinmux)
{
   ...
   {
      LOGD("!!! AnalogAFE_Open FM_PATH_MONO \n");
   ...
   {
      LOGD("!!! AnalogAFE_Open FM_PATH_STEREO \n");
#ifdef ENABLE_DEPOP  //Add by Charlie Lu.
      //Apply Depop Flow
      AnalogAFE_Depop(pinmux,true);
#else
   ...
   {
      LOGD("!!! AnalogAFE_Open FM_MONO_AUDIO \n");
#ifdef ENABLE_DEPOP  //Add by Charlie Lu.
      //Apply Depop Flow
      AnalogAFE_Depop(pinmux,true);
#else
   ...
   {
      LOGD("!!! AnalogAFE_Open FM_STEREO_AUDIO \n");
#ifdef ENABLE_DEPOP  //Add by Charlie Lu.
      //Apply Depop Flow
      AnalogAFE_Depop(pinmux,true);
#else
...
void AudioAnalog::AnalogAFE_Depop(AnalogAFE_Mux pinmux, bool bEnable)
{
    ...
    if (bEnable != false)//depop turn-on sequence
    {
        ...
        switch(pinmux)
        {
          ...
          default:{
              LOGD("AnalogAFE_ChangeMux with default \n");
              break;
          }
        }
        //Restore buffer gain
...
void AudioAnalog::AnalogAFE_Close(AnalogAFE_Mux pinmux)
{
   ...
   if((pinmux == FM_PATH_MONO) || (pinmux == FM_PATH_STEREO) )
   {
      ...
      {
         EnableHeadset (false);
         LOGD("!!! AnalogAFE_Close, FM_PATH_MONO, FM_PATH_STEREO \n");
...
//         SetAnaReg(AUDIO_CON3,0x0002,0x0002);   // audio bias power down
      ...
      {// no voice and Enable FM
         ...
         {
            mAudioHardware->ForceDisableSpeaker();
            LOGW("AnalogAFE_Close GetAnalogLineinEnable, no SPK");
         }
...
{
    LOGD("AnalogAFE_Set_LineIn_Gain gain_value = %d",gain_value);
...
{
   LOGD("HQA_AFE_Set_DL_AUDHPR_PGA_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_AFE_Set_AUD_LineIn_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("Afe_Set_AUD_Level_Shift_Buf_L_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("Afe_Set_AUD_Level_Shift_Buf_R_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_AFE_Set_AUD_UL_ANA_PreAMP_L_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_AFE_Set_AUD_UL_ANA_PreAMP_R_Gain, gain_value:0x%x \n",gain_value);
...
{
   LOGD("HQA_Analog_AFE_Select_Audio_Voice_Buffer, audio_voice_DAC_sel:0x%x \n",audio_voice_DAC_sel);
