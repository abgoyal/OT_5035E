...
#include "AudioIoctl.h"
    ...
    #define LOG_VOLUMECONTROLLER LOGD
#else
    #define LOG_VOLUMECONTROLLER LOGV
#endif
 ...
 int32 AudioYusuVolumeController::AudioGetRoute(uint32 routes)
{
    if(routes <=0)
    {
        LOGE("AudioGetRoute routes = %d",routes);
        return -1;
    }
...
uint32 AudioYusuVolumeController::Calgain(uint16 DegradedBGain,uint32 mode ,uint32 routes)
{
    ...
    LOGD("Calgain DegradedBGain = %x  mode = %d routes = %d",DegradedBGain,mode,routes);
...
uint32 AudioYusuVolumeController::CalVoicegain(uint16 DegradedBGain,uint32 mode ,uint32 routes)
{
    ...
    routes = AudioGetRoute(routes);
    DegradedBGain = (DEVICE_VOLUME_STEP - DegradedBGain) / VOICE_ONEDB_STEP;
    LOGD("CalVoicegain DegradedBGain = %x  mode = %d routes = %d",DegradedBGain,mode,routes);
    if(DegradedBGain >= mVolumeRange[mode][routes]){
        DegradedBGain = mVolumeRange[mode][routes];
    }
    HW_Gain = DegradedBGain <<1; // 0.5db  1 unit
...
void AudioYusuVolumeController::ApplyHandsetGain(uint16 DegradedBGain,uint32 mode,uint32 routes)
{
    ...
    LOGD("::ApplyHandsetGain Setting_value=%x",Setting_value);
...
void AudioYusuVolumeController::ApplyGain(uint16 DegradedBGain,uint32 mode,uint32 routes)
{
    ...
    routes = AudioGetRoute(routes);
    DegradedBGain = (DEVICE_VOLUME_STEP - DegradedBGain) / VOICE_ONEDB_STEP;
    //LOGD("ApplyGain u4Gain = %x  mode = %d routes = %d",DegradedBGain,mode,routes);
    if(DegradedBGain >= mVolumeRange[mode][routes]){
        DegradedBGain = mVolumeRange[mode][routes];
    }
    HW_Gain = mVolumeMax[mode][routes] - DegradedBGain;
    ...
    LOGD("ApplyGain Setting_value=%x",Setting_value);
...
void AudioYusuVolumeController::ApplyGainInCall(uint16 DegradedBGain,uint32 mode,uint32 routes)
{
    ...
    routes = AudioGetRoute(routes);
    DegradedBGain = (DEVICE_VOLUME_STEP - DegradedBGain) / VOICE_ONEDB_STEP;
    //LOGD("ApplyGainInCall u4Gain = %x  mode = %d routes = %d",DegradedBGain,mode,routes);
    if(DegradedBGain >= mVolumeRange[mode][routes]){
        DegradedBGain = mVolumeRange[mode][routes];
    }
    HW_Gain = mVolumeMax[mode][routes] - DegradedBGain;
    ...
    LOGD("ApplyGainInCall, Setting_value=%x HW_Value = %x",Setting_value,HW_Value[Setting_value]);
    mAnaReg->AnalogAFE_Request_ANA_CLK();
...
//        LOG_VOLUMECONTROLLER(":ApplyGain u4Gain=%x, mVolumeRange:%d",u4Gain,mVolumeRange[android_audio_legacy::AudioSystem::MODE_NORMAL][routes]);
    ...
    LOGD(":ApplyGain Setting_value = %d",Setting_value);
...
{
    LOGD("HandleFmFd mfmFd = %d",mfmFd);
    if(Enable && mfmFd <= 0){
        mfmFd = open(FM_DEVICE_NAME, O_RDWR);
        if (mfmFd < 0)
            LOGE("FAIL open mfmFd  failed");
        else{
            LOGD("open(FM_DEVICE_NAME, O_RDWR) success mfmFd = %d",mfmFd);
        }
    }
    else if (Enable && mfmFd >0){
        LOGD("HandleFmFd  Enable and mfmFd = %d",mfmFd);
    }
    else{
        if(mfmFd){
            close(mfmFd);
            mfmFd =0;
            LOGD("close mfmFd and mfmFd = %d",mfmFd);
        }
        else
            LOGE("HandleFmFd mfmFd <0");
    }
    return true;
}
...
static float Mamp255Volumeto0(char Volume)
{
    ...
    LOGD("Mamp255Volumeto0 floatVolume = %f Volume = %d",floatVolume,Volume);
    return floatVolume;
}
...
static float MampAudioVolume(char Volume)
{
    ...
    LOGD("Volume = %d MampAudioVolume DegradedB = %f ",Volume,DegradedB);
    return DegradedB;
}
...
static float MampVoiceVolume(char Volume)
{
    ...
    LOGD("Volume = %d MampVoiceVolume DegradedB = %f ",Volume,DegradedB);
    return DegradedB;
}
...
static float MampUplinkGain(char Volume)
{
    ...
    LOGD("Volume = %dUPLINK_GAIN_MAX DegradedB = %f ",Volume,DegradedB);
    return DegradedB;
}
...
static float MampSidetoneGain(char Volume)
{
    ...
    LOGD("Volume = %d MampSidetoneGain DegradedB = %f ",Volume,DegradedB);
    return DegradedB;
}
...
static float MampLevelShiftBufferGain(char Volume)
{
    ...
    LOGD("Volume = %d MampLevelShiftBufferGain DegradedB = %f ",Volume,DegradedB);
    return DegradedB;
}
...
static float Mamp0Volumeto100(char Volume)
{
    ...
    LOGD("Mamp0Volumeto100 floatVolume = %f Volume = %d",floatVolume,Volume);
    return floatVolume;
}
...
bool AudioYusuVolumeController::InitVolumeController()
{
   ...
   //----------------------------------------------------------
    ...
    LOGD(" not define MTK_AUDIO_GAIN_TABLE_SUPPORT");
    ...
    #else
    LOGD(" MTK_AUDIO_GAIN_TABLE_SUPPORT");
    AUDIO_GAIN_TABLE_STRUCT Gain_Table;
    GetAudioGainTableParamFromNV(&Gain_Table);
    Gain_Table.Microphone_Gain_table.Mic_Gain[IDLE_RECORD_NORMAL] = 120;
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[IDLE_RECORD_NORMAL]);
    SetMicGain(Idle_Normal_Record, -1 * degradegain <<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[IDLE_HEADSET]);
    SetMicGain(Idle_Headset_Record, -1 * degradegain <<1);
    Gain_Table.Microphone_Gain_table.Mic_Gain[INCALL_NORMAL] = 160;
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[INCALL_NORMAL]);
    SetMicGain(Normal_Mic,         -1 *  degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[INCALL_HEADSET]);
    SetMicGain(Headset_Mic,        -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[INCALL_SPEAKER]);
    SetMicGain(Handfree_Mic,       -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[TTY_DEVICE]);
    SetMicGain(TTY_CTM_Mic ,      -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[Analog_RECORD]);
    SetMicGain(Fm_Record ,     -1 *degradegain <<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[SIPCALL_NORMAL]);
    SetMicGain(VOIP_Normal_Mic,    -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[SIPCALL_HEADSET]);
    SetMicGain(VOIP_Headset_Mic,   -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[SIPCALL_SPEAKER]);
    SetMicGain(VOIP_Handfree_Mic, -1 * degradegain<<1);
    degradegain =(unsigned char)MampUplinkGain(Gain_Table.Microphone_Gain_table.Mic_Gain[VOICE_REC]);
    SetMicGain(Voice_Rec_Mic_Handset, -1 * degradegain<<1);

    // init sidetone data
    degradegain =(unsigned char)MampSidetoneGain(Gain_Table.Sidetone_Gain_table.SdieTone_Gain[SIDETONE_NORMAL]);
    SetSideTone( 0   , -1 * degradegain<<1);
    degradegain =(unsigned char)MampSidetoneGain(Gain_Table.Sidetone_Gain_table.SdieTone_Gain[SIDETONE_HEADSET]);
    SetSideTone( 1   , -1 * degradegain<<1);
    degradegain =(unsigned char)MampSidetoneGain(Gain_Table.Sidetone_Gain_table.SdieTone_Gain[SIDETONE_SPEAKER]);
    SetSideTone( 2   , -1 * degradegain<<1);
    #endif
...
bool AudioYusuVolumeController::SetLadMicGain(int MicType)
{
    ...
    LOGD("SetLadMicGain MicMode = %d MicType = %d",MicMode,MicType);
    if(MicMode == android_audio_legacy::AudioSystem::MODE_IN_CALL) // incall mode
    {
        LOGD("SetLadMicGain mMicGain[%d]=%d (%x)",MicType,mMicGain[MicType],mMicGain[MicType]);
    ...
    {
        LOGD("SetLadMicGain mMicGain=%d (%x)",mMicGain[MicType],mMicGain[MicType]);
        mAudioHardware->pLad->LAD_SetMicrophoneVolume( mMicGain[MicType]);
        return true;
    }
    return true;
...
void AudioYusuVolumeController::SetLineInGainByMode(uint32 mode)
{
    switch(mode){
        ...
        case LINE_GAIN_MATV:
            ...
            break;
        default:
            LOGW("SetLineInGainByMode nut sno mode select %d",mode);
            break;
    }
}
...
bool AudioYusuVolumeController::SetApMicGain()
{
    //only in normal mode record
    uint32 Pga_gain =0;
    uint32 degradegain =(uint32)MampUplinkGain(VolumeCustomParam.audiovolume_mic[VOL_HANDFREE][4]);
    Pga_gain = MIC_GAIN_MAX -degradegain;
    if(Pga_gain < MIC_GAIN_MAX){
        Pga_gain = MIC_GAIN_MAX;
    }
    LOGD("SetLadMicGain degradegain  = %d MicType = %d Pga_gain = %d",degradegain,Pga_gain);
...
bool AudioYusuVolumeController::SetLevelShiftBufferGain(){
    ...
    LOGD("SetLevelShiftBufferGain = %d",LevelShiftBufferGain);
    ...
    case (android_audio_legacy::AudioSystem::MODE_IN_CALL):  // in call mode
        {
            ...
            case (android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET) :{
                ...
                uint32 Lad_Volume = Calgain(MapVolume,mode,routes);
                LOGD("setMasterVolume, Lad_Volume = %x",Lad_Volume) ;
                mAudioHardware->pLad->LAD_SetOutputVolume(Lad_Volume);
#else
                ApplyGain(MapVolume,  mode, routes);
#endif
...
bool AudioYusuVolumeController::setVoiceVolume(float volume, uint32 mode,uint32 routes)  // volume should range 0.0 ~ 1.0
{
        ...
        ||GetDevice == ROUTE_OUT_WIRED_HEADPHONE)  // adjust for analog gain
    {
        ...
        //LOGD("setVoiceVolume sidetone:%x [...ApplyGainInCall]",mSideTone[GetDevice]);
    ...
    {
        LOGW("setVoiceVolume with ROUTE_OUT_BLUETOOTH_SCO");
        return true;
    }
    else
    {
        LOGW("setVoiceVolume with no default");
        return true;
    }
    return false;
}
...
bool AudioYusuVolumeController::setVoIPVolume(float volume, uint32 mode,uint32 routes)  // volume should range 0.0 ~ 1.0
{
        ...
        ||GetDevice == ROUTE_OUT_WIRED_HEADPHONE)  // adjust for analog gain
    {
        ...
        ||GetDevice == ROUTE_OUT_BLUETOOTH_SCO_CARKIT)
    {
        LOGW("setVoIPVolume with ROUTE_OUT_BLUETOOTH_SCO");
        return true;
    }
    else
    {
        LOGW("setVoIPVolume with no default");
        return true;
    }
    return false;
}
...
{
   int ret=0;
   LOGD("!!SetFmVolume=%d, mfmFd=%d",volume,mfmFd);
...
// if use MT519x FM, don't cotnrol the MT6620 FM volume
   ...
   LOGD("!!MT6620, SetFmVolume(%d)",volume);
...
// ToDo
   LOGD("!!MT519x, SetFmVolume(%d)",volume);
   if(spATVCtrlService == NULL){
      LOGD("SetFmVolume, spATVCtrlService(NULL)");
      GetMatvService ();
      if(spATVCtrlService == NULL){
         LOGD("SetFmVolume, fail mATV service");
         return false;
      }
   }
   spATVCtrlService->ATVCS_matv_adjust(MATV_AUD_VOLUME, volume);
#else
   LOGD("SetFmVolume [No this FM chip]");
#endif
...
{
    LOGD("setMatvVolume volume=%d",volume);

#ifdef MATV_AUDIO_LINEIN_PATH
    if(spATVCtrlService == NULL){
        LOGW("setMatvVolume but spATVCtrlService == NULL");
        GetMatvService ();
        if(spATVCtrlService == NULL){
            LOGE("setMatvVolume cannot get matv service");
            return false;
        }
    }
...
{
   LOGD("SetMatvMute(%d), mMatvVolume(%d)",b_mute,mMatvVolume);

#ifdef MATV_AUDIO_LINEIN_PATH
   if(spATVCtrlService == NULL){
     LOGW("SetMatvMute, can't get spATVCtrlService");
     GetMatvService ();
     if(spATVCtrlService == NULL){
         LOGE("SetMatvMute cannot get matv service");
         return false;
     }
   }
