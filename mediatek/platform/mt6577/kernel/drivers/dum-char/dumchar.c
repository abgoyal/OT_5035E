...
static int init_region_info(void){
	...
	/*if(__simple_sd_ioctl_get_excsd(&cmd,ext_csd))
	...
	User_Region_Size_Byte = (u64)(ext_csd[212+0]<<0| ext_csd[212+1]<< 8 |ext_csd[212+2]<<16| ext_csd[212+3]<<24)*512;//from emmc spec , only for emmc 4.1-4.41
	printk("[Dumchar_init_region_info]:User_Region_Size_Byte=%llu KB\n",User_Region_Size_Byte/1024);
	...
	for(i=0;i<REGION_NUM;i++){
		...
		printk("[Dumchar_init_region_info]:region %d  size=%lld KB \n",region_info[i].region,region_info[i].size_Byte/1024);
	}

	printk("[Dumchar_init_region_info]:Boot 1 Region + Boot 2 Region + RPMB Region = %llu KB\n",User_region_Header_Byte/1024);

	emmc_size = User_Region_Size_Byte + User_region_Header_Byte;
	printk("emmc size = %llx\n",emmc_size);
	return 0;	
}
static int eMMC_rw_x(loff_t addr,
						u32  *buffer, int host_num, int iswrite,
						u32 totalsize, int transtype, Region part)
{
	struct msdc_ioctl cmd;
	int result = 0;
...
int dumchar_probe(struct platform_device * dev)
{
	...
	//User_Region_Size_Byte=3198512*1024;
	//User_region_Header_Byte=2176*1024;
	//remove
	...
	if(User_Region_Size_Byte < 0){
		printk("[Dumchar]Error User_Region_Size_Byte=%llx\n",User_Region_Size_Byte);
	}
	printk("[Dumchar]MBR  Physical address in User Region=%llu KB, Varity Partition(%s) size=%llu KB\n",(MBR_START_ADDR - User_region_Header_Byte)/1024,PartInfo[zeroIndex].name,User_Region_Size_Byte/1024);
#endif
