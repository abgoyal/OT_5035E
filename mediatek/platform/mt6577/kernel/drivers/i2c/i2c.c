...
static struct mutex mt_i2c_transfer_mutex;
...
static int mt_i2c_transfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if(i2c->id == 2){	//PMIC use only.
	  mutex_lock(&mt_i2c_transfer_mutex);
	    ...
	    for (retry = 0; retry < 10; retry++) {
	        ...
	        udelay(100);
			  ...
			  mutex_unlock(&mt_i2c_transfer_mutex);
		/*PMIC 3 times retry fail, reboot system*/
		if((retry >= 10) && (ret == -EAGAIN)){
...
#endif		
	mutex_init(&mt_i2c_transfer_mutex);
