...
#include <linux/spinlock.h> //legen 0922
...
static PDPI_REGS const DPI_REG = (PDPI_REGS)(DPI_BASE); //legen 0922
...
bool is_needReConfig = true; //legen 0922
...
void DSI_PHY_clk_setting(unsigned int div1, unsigned int div2, unsigned int lane_no)
{
	...
	_dsiContext.bit_time_ns = 1000 * (1000 * (div2 ? div2*0x02 : 0x1)) / (26 * (div1+0x01)) / lane_no; //legen 0922
...
void DSI_handle_esd_recovery(void)
{
	...
	if (dsi_noncont_clk_enabled && (dsi_dpi_isr_count % dsi_noncont_clk_period)==0) {
		...
		{	
			...
			//printk("--------%s: noncount---state=%0x----\n",__func__,state);//baoqiang
...
void DSI_set_cmdq_V2(unsigned cmd, unsigned char count, unsigned char *para_list, unsigned char force_update)
{
	...
	if (count > 59)
	{
		...
		bool			lcd_w2m, lcm_te_enable; //legen 0922
...
//		DSI_Reset();
		...
		LCD_REG->DS_DSI_CON.PACKET_SIZE = 256; //30 legen 0922

		// Disable TE
		LCD_TE_Enable(0); //legen 0922
		...
		// operates on FB layer
		{
		    ...
		    //LCD_REG->LAYER[FB_LAYER].ADDRESS = fbPhysAddr; //legen 0922
		}
#endif
