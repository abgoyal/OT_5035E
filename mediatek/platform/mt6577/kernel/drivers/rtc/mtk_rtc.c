...
#include <rtc-mt6577.h>		/* custom file */

#define XLOG_MYTAG	"Power/RTC"
#define RTC_NAME	"mt6577-rtc"
...
static irqreturn_t rtc_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(MT6577_RTC_IRQ_ID);
...
static int rtc_pdrv_probe(struct platform_device *pdev)
{
	...
	r = request_irq(MT6577_RTC_IRQ_ID, rtc_irq_handler, IRQF_TRIGGER_LOW,
	                RTC_NAME, NULL);
	if (r) {
...
static int __init rtc_late_init(void)
{
	...
	pdn1 = rtc_read(RTC_PDN1);
	spar0 = rtc_read(RTC_SPAR0);
	rtc_write(RTC_SPAR0, spar0 & ~0x8000);	/* clear DPIDLE_SHUTDOWN bit */
	rtc_write_trigger();
	spin_unlock_irqrestore(&rtc_lock, flags);

	rtc_xerror("RTC_IRQ_EN = 0x%x, RTC_PDN1 = 0x%x, RTC_SPAR0 = 0x%x\n",
	           irqen, pdn1, spar0);
...
MODULE_DESCRIPTION("MT6577 RTC Driver v3.1");
MODULE_LICENSE("GPL");
