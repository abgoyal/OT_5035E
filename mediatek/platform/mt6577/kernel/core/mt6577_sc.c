...
/* use RTC_SPAR0[15] to mark if normally exit deep idle with shutdown mode */
#define SC_DPIDLE_SHUTDOWN_DEBUG

#ifdef SC_DPIDLE_SHUTDOWN_DEBUG
...
#ifdef SC_DPIDLE_SHUTDOWN_DEBUG
static void sc_wait_for_interrupt_dpidle(unsigned int pwrlevel)
{
	u32 ckdiv1;
	...
	if (pwrlevel == 0) {
		...
		rtc_write(RTC_SPAR0, rtc_read(RTC_SPAR0) | 0x8000);
...
#else
#define sc_wait_for_interrupt_dpidle(pwrlevel)	\
...
void sc_go_to_dpidle(unsigned int pwrlevel)
{
	...
	if (sc_dpidle_can_enter()) {
		...
		sc_wait_for_interrupt_dpidle(pwrlevel);
...
MODULE_DESCRIPTION("MT6577 SLPCTRL Driver v4.4");
