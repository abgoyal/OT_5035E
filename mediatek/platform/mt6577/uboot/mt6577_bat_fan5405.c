...
void fan5405_set_ac_current(void)
{
    ...
    //4). 0x04h->0x19h
    //5). 0x05h->0x04h
    fan5405_config_interface_liao(0x06,0x70); //set ISAFE :850mA, 4.2V  mingquan.lai modify for Camry PIO
    fan5405_config_interface_liao(0x01,0xC8);
    fan5405_config_interface_liao(0x02,0x8E);
    fan5405_config_interface_liao(0x04,0x79); //set IOCHARGE mingquan.lai modify for Camry PIO
    fan5405_config_interface_liao(0x05,0x04);
    #endif
}
...
void ChargerHwInit_fan5405(void)
{
	...
	if(temp_init_flag==0)
	{
	    fan5405_config_interface_liao(0x06,0x70);	/*mingquan.lai modify for Camry	PIO*/
...
#if 0  //mingquan.lai cancel for Camry        
        /* UBOOT charging LED */
...
void check_battery_exist(void)
{
	...
	#if 0	/*FIXME don't power off when battery id error*/
        pchr_turn_off_charging_fan5405();
        mt6577_power_off();        
	#endif
    }
   ...
   // if(upmu_chr_get_vcdt_hv_det() == 1 || (upmu_is_chr_det() == KAL_FALSE))
    ...
    else
    {
       ...
       if(BMT_status.bat_vol < VOL_TOP_OFF)
       {
    	...
    	if( (fan5405_status == 0x2) || (BMT_status.TOPOFF_charging_time > SHOW_FULL_TIME) )
    	{
    		BMT_status.bat_charging_state = CHR_BATFULL;
    		BMT_status.bat_full = KAL_TRUE;
    		BMT_status.total_charging_time = 0;
...
#endif
     {
		...
		fan_fault_status = fan5405_get_fault_reason();	/**/
		   ...
		   mt65xx_leds_brightness_set(6, 20);
                ...
                BMT_status.charger_vol = PMIC_IMM_GetOneChannelValue(AUXADC_CHARGER_VOLTAGE_CHANNEL, 5);
                BMT_status.charger_vol = BMT_status.charger_vol / 100;
		 if (BMT_status.charger_vol < 4000)     
		 {             
		    #ifndef NO_POWER_OFF                
		    mt6577_power_off();              
		    #endif             
...
{    
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))
    {	
		mt_set_gpio_mode(GPIO67, GPIO_MODE_00);//mingquan.lai add for Camry:led off
 		mt_set_gpio_dir(GPIO67, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO67, GPIO_OUT_ONE);
