...
static bool lcd_esd_check = false; //legen 0922

static int esd_test_num = 0; //baoqiang
...
{   
    ...
    if (status.SYNC)// this is TE mode 0 interrupt
    {
        ...
        DBG_OnTeDelayDone();
		lcd_esd_check = false; //legen 0922
    }
#if 0  //TE mode 1
	if(status.HTT)
	{
            ...
            if (0 == ret) {
				...
				if(_IsEngineBusy()){
					...
					if(LCD_REG->STATUS.WAIT_SYNC){
						...
						//DSI_handle_esd_recovery(METHOD_LCM_VSYNC); //baoqiang 0922
						//DSI_handle_esd_recovery(); //this is modify by baoqiang
						lcd_esd_check = true; //legen 0922					
					}
...
BOOL LCD_esd_check(void)
{
	...
	/*
	printk("-----qbq---%s---start----esd_test_num=%d----\n",__func__,esd_test_num);
	esd_test_num ++;
	if(esd_test_num == 20)
	{
		lcd_esd_check = 1;
	...
	*/
	...
	if(esd_test_num ==200)
	{
		printk("-----qbq---%s---end-----lcd_esd_check=%d---\n",__func__,lcd_esd_check);
		esd_test_num = 0;
	}
