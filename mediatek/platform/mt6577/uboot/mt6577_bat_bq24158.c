...
#include <cust_gpio_usage.h>   //mtk71259 20120712 DWS
...
void select_charging_curret_bq24158()
{
    ...
    {        
		...
		bq24158_config_interface_reg(0x04,0x29); //set IOCHARGE  to 750mA for USB fast chg test
		if (Enable_BATDRV_LOG == 1) {
...
void check_battery_exist(void)
{
    ...
    if( baton_count >= 3)
    {
        ...
        {
        printf("[BATTERY] Battery is not exist, power off bq24158 and system (%d)\n", baton_count);
        pchr_turn_off_charging_bq24158();
        mt6577_power_off();   
		}
    }
        ...
        while (1) 
        {
            	...
            	|| mtk_wdt_boot_check()==WDT_BY_PASS_PWK_REBOOT)			
            {
                ...
                if(back_to_charging_animation_flag == false)
		        {
                    ...
                    //restore
                    printf("Restore GPIO_SWCHARGER_EN_PIN=%d \n", gpio_on_out);
                    mt_set_gpio_out(GPIO_SWCHARGER_EN_PIN, gpio_on_out);
