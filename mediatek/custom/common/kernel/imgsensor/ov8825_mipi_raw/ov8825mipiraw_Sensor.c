...
kal_uint16 read_OV8825_gain(void)
{
	...
	ov8825.sensorGlobalGain = (((OV8825_read_cmos_sensor(0x350a)&0x01) << 8) | OV8825_read_cmos_sensor(0x350b)); 
...
void OV8825_sensor_to_camera_para(void)
{
    ...
    for(i=0; 0xFFFFFFFF!=OV8825SensorReg[i].Addr; i++)
    {
        OV8825SensorReg[i].Para = OV8825_read_cmos_sensor(OV8825SensorReg[i].Addr);
    }
    for(i=ENGINEER_START_ADDR; 0xFFFFFFFF!=OV8825SensorReg[i].Addr; i++)
    {
        OV8825SensorReg[i].Para = OV8825_read_cmos_sensor(OV8825SensorReg[i].Addr);
...
{	
	...
	OV8825_write_cmos_sensor(0x380e,0x05);//VTS = 1264  /    1310         
...
static void OV8825_Sensor_Init(void)
{
	...
	OV8825_write_cmos_sensor(0x361c, 0x04); //VCM output current, add by ruby
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	OV8825_write_shutter(ov8825.pvShutter);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ov8825.sensorGlobalGain = read_OV8825_gain();
...
UINT32 OV8825SetVideoMode(UINT16 u2FrameRate)
{
    ...
    if(ov8825.sensorMode == SENSOR_MODE_PREVIEW)
    {
		...
		ov8825.DummyLines = MAX_Frame_length - OV8825_PV_PERIOD_LINE_NUMS;
	...
	else if(ov8825.sensorMode == SENSOR_MODE_CAPTURE)
	{
		...
		ov8825.DummyLines = MAX_Frame_length - OV8825_FULL_PERIOD_LINE_NUMS;
