...
int fts_ctpm_auto_upg(void)
{
	...
	if((tp_vendor != 0x5a)	&& (tp_vendor != 0x53))
	{
	...
	if(tp_vendor == 0x5a){
		...
		printk("[FT5316] tp vendor is Truly(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		...
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		printk("[FT5316] tp vendor is Mutto(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		for (i = 0; i < sizeof(version_list_camry_mutto)/sizeof(version_list_camry_mutto[0]); i++)
		{
			if (uc_tp_fm_ver == version_list_camry_mutto[i])
				...
				if (i_ret == 0)
	    		{
	     			printk("[FT5316] Mutto tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
	    		}
				else
	   			{
	    			printk("[FT5316] Mutto tp firmware upgrade failed ret=%d.\n", i_ret);
	    		}
				break;
			}
		}		
