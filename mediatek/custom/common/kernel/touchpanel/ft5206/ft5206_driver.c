...
reset_proc:   
  ...
  mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
    ...
    ret = i2c_smbus_read_i2c_block_data(i2c_client, FT5X0X_REG_FIRMID, 1, &uc_reg_value);
    printk("mtk_tpd: Firmware version = 0x%x\n", uc_reg_value);
	ret = i2c_smbus_read_i2c_block_data(i2c_client, FT5X0X_REG_PERIODACTIVE, 1, &uc_reg_value);
    printk("mtk_tpd: report rate is %dHz.\n", uc_reg_value * 10);
	ret = i2c_smbus_read_i2c_block_data(i2c_client, FT5X0X_REG_THGROUP, 1, &uc_reg_value);
    printk("mtk_tpd: touch threshold is %d.\n", uc_reg_value * 4);
	ret = i2c_smbus_read_i2c_block_data(i2c_client, FT5X0X_REG_FT5201ID, 1, &uc_reg_value);
	if(uc_reg_value == 0x5a){
		printk("mtk_tpd: tp vendor is Truly.\n");
	}
	else if(uc_reg_value == 0x53)
	{
		printk("mtk_tpd: tp vendor is Mutto.\n");
	}
	else
		printk("mtk_tpd: tp vendor is unknown.\n");
	tpd_load_status = 1;
