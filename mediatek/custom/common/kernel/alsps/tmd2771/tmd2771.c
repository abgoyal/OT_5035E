...
#if 0//lipeng tmp debug
static int cal_params[4] = {0};//max, mean, high, low
#endif
/*----------------------------------------------------------------------------*/
...
#if 1//Enabled by jrd.lipeng for boot-up calibration
...
//	u16 ps_value;    
	...
	*data = ps_value_low[0] | (ps_value_high[0]<<8);
	...
	//printk("jrd.lipeng|ps=%d, high_thd:%d  low_thd:%d\n", *data, atomic_read(&tmd2771_obj->ps_thd_val_high), atomic_read(&tmd2771_obj->ps_thd_val_low));
        //printk("jrd.lipeng|calibration: max=%d, mean=%d, high=%d, low=%d\n", cal_params[0], cal_params[1], cal_params[2], cal_params[3]);//lipeng tmp debug
	return 0;    
...
static void tmd2771_ps_calibrate(struct i2c_client *client)
{
	...
	for(i = 0; i < 20; i++)
	{
		...
		mdelay(60);//160
#ifdef JRD_DEBUG
		printk(KERN_ERR "yuanjun tmd2771_ps_calibrate  ps-val = %d XXXXX\n", prox_data[i]);
#endif 
	}
	...
	prox_threshold_hi = prox_mean*45/10;
	prox_threshold_lo = prox_mean*16/10;
	}
	else if(prox_mean<150){
	prox_threshold_hi = prox_mean*4;
	prox_threshold_lo = prox_mean*15/10;
	}
	else if(prox_mean<200){
	prox_threshold_hi = prox_mean*35/10;
	prox_threshold_lo = prox_mean*15/10;
	}
